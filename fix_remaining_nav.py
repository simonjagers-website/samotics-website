...
for f in FILES:
    ...
    original = content
    if OLD_PLATFORM_DESKTOP in content:
        content = content.replace(OLD_PLATFORM_DESKTOP, ASSETS_DESKTOP, 1)
    if OLD_MOBILE_PLATFORM in content:
        content = content.replace(OLD_MOBILE_PLATFORM, NEW_MOBILE, 1)
    if content != original:
